...
enum ProFeature: Equatable, Hashable {
    ...
    var isAvailable: Bool { AltTabNeoFreeMode.enabled || LicenseManager.shared.isProAvailable }
    /// True when Pro is locked (post-expiration).
    var isLocked: Bool { !AltTabNeoFreeMode.enabled && LicenseManager.shared.isProLocked }
    ...
    static func isStoredValuePro(preferenceKey: String) -> Bool {
        ...
        return ProGatedPreferences.forPreferenceKey(preferenceKey)?.isStoredValuePro() ?? false
    }
}
