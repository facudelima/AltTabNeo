...
enum ProFeature: Equatable, Hashable {
    ...
    var isAvailable: Bool { AltTabXFreeMode.enabled || LicenseManager.shared.isProAvailable }
    /// True when Pro is locked (post-expiration).
    var isLocked: Bool { !AltTabXFreeMode.enabled && LicenseManager.shared.isProLocked }
