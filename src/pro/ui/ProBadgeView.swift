...
class ProBadgeView: NSView {
    ...
    static func attach(to segmentedControl: NSSegmentedControl, segmentIndex: Int, label: String, symbol: Symbols) -> SegmentOverlay {
        if AltTabNeoBranding.hideProUi {
            ...
            let hiddenIcon = NSImageView()
            hiddenIcon.isHidden = true
            let hiddenLabel = NSTextField(labelWithString: "")
            hiddenLabel.isHidden = true
            return SegmentOverlay(badge: hiddenBadge, icon: hiddenIcon, label: hiddenLabel)
        }
    ...
    override init(frame frameRect: NSRect) {
        ...
        if !ProBadgeView.proLabel.isEmpty {
